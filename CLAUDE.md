...
1. **`planner`** (LLM) — in one call, selects 1–3 relevant coding systems **and** generates one search term per selected system. On refinement, re-entered with the prior attempt's results as context, so it can revise both system selection and search terms jointly. If `selected_systems` is empty (miss/gibberish query), the graph short-circuits directly to re_ranker — executor and evaluator are skipped.
2. **`executor`** (async fan-out) — calls only the selected Clinical Tables APIs concurrently; per-system failures are isolated.
3. **`evaluator`** (LLM) — decides *sufficient* (forward) or *refine* (loop back to planner); also populates `relevant_codes` (per-system list of codes matching the clinical intent) on every pass. **Capped at 2 iterations** — enforced in graph state, not in the prompt. On iteration 2+, the evaluator prompt includes a "carried over" block of accumulated results from systems that were strong in prior iterations and not re-queried — coverage is assessed against the combined set to avoid false-positive refine.
4. **`re_ranker`** (LLM) — applies evaluator's `relevant_codes` domain filter per system, pools all results into a flat list, then calls an LLM to rank by query relevance. Returns a flat `list[CodeResult]` ordered most-to-least relevant (max `flat_results = 5`). Short-circuits without an LLM call for empty or small pools.
...
- **Refinement only fires on weak results** (empty results for a planner-selected system, or results that don't match the clinical intent of the query). Not on every query.
