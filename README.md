...
**Longer-term:**
- **Improve multi-system recall** — the biggest current gap (top-3 recall 0.25 for multi-system queries). The planner's conservative defaults help precision on simple queries but under-select on queries that genuinely span multiple domains (e.g. "hypertension management", "diabetes management"). Options: loosen the multi-domain trigger threshold, add a second query term per system, or use a two-pass approach where the first pass selects systems and the second generates terms per system with more context.
- **Multi-turn support** — currently single-turn only; state resets per query. Conversational follow-up ("now narrow to type 2 only", "show me just the lab codes") would significantly improve usability for clinical workflows.
- **Cost optimization** — split the planner into a deterministic router + LLM planner (cheap rules handle 80% of unambiguous queries; LLM fires only on the ambiguous tail), and replace the LLM evaluator with a deterministic policy for clear-cut cases (zero results) reserving the LLM call for genuinely ambiguous domain-match judgments.
- **LangSmith tracing** for production observability — node-level inputs/outputs, latency, and token usage per query.
